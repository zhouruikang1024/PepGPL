...
def get_train_config():
    parse = argparse.ArgumentParser(description='PepGPL train model')

    # preoject setting
    parse.add_argument('--path-dataset', type=str, default='../dataset/pep_prot_label_dataset_shuffle.csv',
                       help='dataset path')
    parse.add_argument('--setting', type=str, default='new_peptide', help='run program under different setting')
    parse.add_argument('--clu-thre', type=float, default=0.3, help='cluster thresold')
    parse.add_argument('--prot_idx', type=str, default='../preprocessing/prot_seq_index.csv')
    parse.add_argument('--pep_idx', type=str, default='../preprocessing/pep_seq_index.csv')
    ...
    parse.add_argument('--num_node', type=int, default=64, help='the number of super_nodes')
    # model parameters
    parse.add_argument('--pad-pep-len', type=int, default=50, help='max peptide length')
    parse.add_argument('--pad-pro-len', type=int, default=676, help='max protein length')
    ...
    # parse.add_argument('--gnn-layer', type=int, default=2, help='number of gnn layer')

    # training parameters
    parse.add_argument('--num-worker', type=int, default=2, help='dataloader number_worker')
    ...
    parse.add_argument('--epoch', type=int, default=40, help='number of iteration')
    parse.add_argument('--k-fold', type=int, default=5, help='k in cross validation')
    ...
    parse.add_argument('--device', type=str, default='cuda:0', help='device id')
    ...
    parse.add_argument('--interval-valid', type=int, default=1,
                       help='how many epochs have gone through to record the validation performance')
    parse.add_argument('--interval-test', type=int, default=1,
                       help='how many epochs have gone through to record the test performance')
